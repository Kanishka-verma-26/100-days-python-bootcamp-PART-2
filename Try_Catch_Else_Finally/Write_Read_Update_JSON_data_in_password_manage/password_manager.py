...
def save_entry():
    ...
    # creating dictionary to dump into json
    new_data = {
        web: {
            "email" : em,
            "password" : ps,
...
def find_pass():
    ...
    try:
        ...
        messagebox.showinfo(title="Ero Data File Found.")
    else:
        if web in data:
            em = data[web]["email"]
...
# search button
search = Button(text="Search", width=15, highlightthickness=0, bg="white", command=find_pass)
...
# add button
add = Button(text="Add", highlightthickness=0, width=37, bg="white", command=save_entry)
